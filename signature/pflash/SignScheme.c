...
int pflash_keypair( unsigned char sk[SECRETKEY_BYTES] , unsigned
...
int pflash_signature (
  ...
  unsigned char sk[SECRETKEY_BYTES],const unsigned long long sklen )
{
...
int pflash_verification (
    const unsigned char m[SHORTHASH_BYTES],const unsigned long long mlen,
    unsigned char sm[SIGNATURE_BYTES],const unsigned long long smlen,
    unsigned char pk[PUBLICKEY_BYTES],const unsigned long long pklen )
{
	...
	// for(int i=0;i<24;i++) printf("%x ",h[i]); printf("\n");
	return 0;
}
