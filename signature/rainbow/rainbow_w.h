...
template <unsigned p>
void rainbow_rand_gfv(rainbow_gfv<p> *vec){
	for(unsigned i = 8; i--;)
		((rainbow_gf<p> *)(&(vec->v)))[i] = rainbow_gf<p>::rand();
}

template <unsigned p, unsigned m, unsigned n>
void rainbow_rand_matrix( MLPS<p,m,n> * mat )
{
	for( unsigned i = mat->M ; i-- ; )
		for( unsigned j = ( mat->N * 8 ) ; j-- ; ) 
			rainbow_rand_gfv( &(mat->l[i][j]) );
	zero_vec( mat );
}

template <unsigned p, unsigned m, unsigned n>
void rainbow_rand_quad( MQPS<p,m,n> * poly )
{
	rainbow_rand_matrix( poly );
	for( unsigned i = poly->M ; i-- ; )
		for( unsigned j = n*(n+1)/2 ; j-- ; )
			rainbow_rand_gfv( &(poly->q[i][j]) );
}
...
void rainbow_w_56_40<p>::gen_q_key( uint8 *  pri_key  )
{
	...
	rainbow_rand_quad( & l0_v );
	rainbow_rand_quad( & l1_v );
	rainbow_rand_quad( & l2_v );
	rainbow_rand_matrix( &l0_o );
	rainbow_rand_matrix( &l1_o );
	rainbow_rand_matrix( &l2_o );
	for( unsigned i=0 ; i < 16 ; i++ ) { rainbow_rand_matrix(&l0_vo[i]); rainbow_rand_matrix(&l2_vo[i]); }
	for( unsigned i=0 ; i < 8 ; i++ ) rainbow_rand_matrix(&l1_vo[i]);
...
void rainbow_w_56_40<p>::q_map( VEC<p,l_sec> *y , const uint8 *  pri_key  , const VEC<p,l_msg> *x )
{
	...
	VEC<p,16> o0; memcpy( & o0 , &((*x)[16])	 , sizeof(rainbow_gf<p>[16]) );
	VEC<p, 8> o1; memcpy( & o1 , &((*x)[32])	 , sizeof(rainbow_gf<p>[8]) );
	VEC<p,16> o2; memcpy( & o2 , &((*x)[40])	 , sizeof(rainbow_gf<p>[16]) );
	VEC<p,16> tmp,tmp2;
	VEC<p, 8> tmp8,tmp28;
	MLPS<p,16,16> o_mat;
	MLPS<p,8,8> o_mat8; 
	...
	eval( & tmp , & l0_v , v0 );
	tmp += tmp2; tmp.reduce();
	memcpy( y , & tmp , sizeof(rainbow_gf<p>[16]) );
	...
	memcpy( &((*y)[16]) , & tmp8 , sizeof(rainbow_gf<p>[8]) );
	...
	eval( & tmp , & l2_v , v2 );
	tmp += tmp2; tmp.reduce();
	memcpy( &((*y)[24]) , & tmp , sizeof(rainbow_gf<p>[16]) );
...
int rainbow_w_56_40<p>::inv_q_map( VEC<p,l_msg> *y , const uint8 *  pri_key  , const VEC<p,l_sec> *x )
{
	...
	{ eval(&tmp,&l0_vo[i],v0); for(unsigned j=tmp.M;j--;) o_mat.l[j][i]+=tmp.c[j]; }
		eval( & tmp , & l0_v , v0 );
		memcpy( & sol , x , sizeof(rainbow_gf<p>[16]) );
		sol -= tmp;
		sol.reduce();
		if( ! rainbow_wiedemann<16>( tmp , o_mat , sol ) ) continue;
		memcpy( &((*y)[16]) , & tmp , sizeof(rainbow_gf<p>[16]) );

	memcpy(&o_mat8,&l1_o,sizeof(MLPS<p,8,8>));
	for( unsigned i=0;i<8;i++) 
	{ eval(&tmp8,&l1_vo[i],v1); for(unsigned j=tmp8.M;j--;) o_mat8.l[j][i]+=tmp8.c[j]; }
		eval( & tmp8 , & l1_v , v1 );
		memcpy( & sol8 , &((*x)[16]) , sizeof(rainbow_gf<p>[8]) );
		sol8 -= tmp8;
		sol8.reduce();
		if( ! rainbow_wiedemann<8>( tmp8 , o_mat8 , sol8 ) ) continue;
		memcpy( &((*y)[32]) , & tmp8 , sizeof(rainbow_gf<p>[8]) );
			
	memcpy(&o_mat,&l2_o,sizeof(MLPS<p,16,16>));
	for( unsigned i=0;i<16;i++) 
	{ eval(&tmp,&l2_vo[i],v2); for(unsigned j=tmp.M;j--;) o_mat.l[j][i]+=tmp.c[j]; }
		eval( & tmp , & l2_v , v2 );
		memcpy( & sol , &((*x)[24]) , sizeof(rainbow_gf<p>[16]) );
		sol -= tmp;
		sol.reduce();
		if( ! rainbow_wiedemann<16>( tmp , o_mat , sol ) ) continue;
		memcpy( &((*y)[40]) , & tmp , sizeof(rainbow_gf<p>[16]) );
...
void rainbow_w_56_40<p>::gen_key( uint8 * pub_key , uint8 * pri_key )
{
	...
	VEC<p, l_msg> x;
	rainbow_gfv<p> _polys[VEC<p, l_sec>::M][(l_msg+1)*(l_msg+2)/2+5];
	{
		VEC<p, l_sec> y;
		zero_vec(&x);
	...
	short *_x = (short *) x.c;
	rainbow_gfv<p> half = invert((rainbow_gf<p>)2);
...
void rainbow_w_64_40<p>::gen_q_key( uint8 *  pri_key  )
{
	...
	rainbow_rand_quad( & l1_v );
	rainbow_rand_matrix( &l1_o );
	for( unsigned i=0 ; i < 20 ; i++ ) { rainbow_rand_matrix(&l0_vo[i]); rainbow_rand_matrix(&l1_vo[i]); }
...
void rainbow_w_64_40<p>::q_map( VEC<p,l_sec> *y , const uint8 *  pri_key  , const VEC<p,l_msg> *x )
{
	...
	VEC<p,20> o0; memcpy( & o0 , &((*x)[24])	 , sizeof(rainbow_gf<p>[20]) );
	VEC<p,20> o1; memcpy( & o1 , &((*x)[44])	 , sizeof(rainbow_gf<p>[20]) );
	VEC<p,20> tmp,tmp2;
	...
	eval( & tmp , & l0_v , v0 );
	tmp += tmp2; tmp.reduce();
	memcpy( y , & tmp , sizeof(rainbow_gf<p>[20]) );
	...
	eval( & tmp , & l1_v , v1 );
	tmp += tmp2; tmp.reduce();
	memcpy( &((*y)[20]) , & tmp , sizeof(rainbow_gf<p>[20]) );
...
int rainbow_w_64_40<p>::inv_q_map( VEC<p,l_msg> *y , const uint8 *  pri_key  , const VEC<p,l_sec> *x )
{
	...
	{ eval(&tmp,&l0_vo[i],v0); for(unsigned j=tmp.M;j--;) o_mat.l[j][i]+=tmp.c[j]; }
		...
		eval( & tmp , & l0_v , v0 );
		memcpy( & sol , x , sizeof(rainbow_gf<p>[20]) );
		sol -= tmp;
		sol.reduce();
		if( ! rainbow_wiedemann<20>( tmp , o_mat , sol ) ) continue;
		memcpy( &((*y)[24]) , & tmp , sizeof(rainbow_gf<p>[20]) );
	...
	{ eval(&tmp,&l1_vo[i],v1); for(unsigned j=tmp.M;j--;) o_mat.l[j][i]+=tmp.c[j]; }
		...
		eval( & tmp , & l1_v , v1 );
		memcpy( & sol , &((*x)[20]) , sizeof(rainbow_gf<p>[20]) );
		sol -= tmp;
		sol.reduce();
		if( ! rainbow_wiedemann<20>( tmp , o_mat , sol ) ) continue;
		memcpy( &((*y)[44]) , & tmp , sizeof(rainbow_gf<p>[20]) );
