#ifndef _RAINBOW_BQAS_H
#define _RAINBOW_BQAS_H
...
template <unsigned p, unsigned m> struct VEC{
	...
	inline VEC<p,m>& operator+= (const VEC<p,m> &b){for (unsigned i = M; i--;) c[i] += b.c[i]; return *this;}
	inline VEC<p,m>& operator-= (const VEC<p,m> &b){for (unsigned i = M; i--;) c[i] -= b.c[i]; return *this;}
	inline VEC<p,m>& operator*= (const rainbow_gf<p> &b){rainbow_gfv<p> _b = b; for (unsigned i = M; i--;) c[i] *= _b; return *this;}
	inline VEC<p,m>& operator/= (const rainbow_gf<p> &b){rainbow_gfv<p> _b = invert(b); for (unsigned i = M; i--;) c[i] *= _b; return *this;}

	inline const VEC<p,m> operator+ (const VEC<p,m> &b) const {VEC<p,m> r = *this; return r+=b;}
	inline const VEC<p,m> operator- (const VEC<p,m> &b) const {VEC<p,m> r = *this; return r-=b;}
	inline const VEC<p,m> operator* (const rainbow_gf<p> &b) const {VEC<p,m> r = *this; return r*=b;}
	inline const VEC<p,m> operator/ (const rainbow_gf<p> &b) const {VEC<p,m> r = *this; return r/=b;}

	inline rainbow_gf<p>& operator[](unsigned i) {return ((rainbow_gf<p>*)c)[i];}
	inline const rainbow_gf<p>& operator[](unsigned i) const {return ((const rainbow_gf<p>*)c)[i];}
...
template <unsigned p, unsigned m, unsigned n> void eval(VEC<p,m> *r, const MQPSv2<p,m,n> *poly, const VEC<p,n> *w){
	...
	*_wij++ = 1;
	...
	rainbow_gfv<p> *res = &r->c[VEC<p,m>::M];
	...
	for (unsigned k = MQPSv2<p,m,n>::M; k--;){
		...
		if (k & 1)
			...
			*--res = reducev((rainbow_gfv<p>)_mm_packs_epi32(acc, acchi));
	}
}
...
void rand_vec(VEC<p, n> *vec){
	for(unsigned i = n; i--;)
		((rainbow_gf<p> *)vec->c)[i] = rainbow_gf<p>::rand();
}
...
template <unsigned p, unsigned m, unsigned n>
static void mat_col_scale(MLPS<p, m, n> *mat, unsigned col, rainbow_gf<p> s){
	rainbow_gfv<p> sv = s;
...
template <unsigned p, unsigned m, unsigned n>
static void mat_col_add(MLPS<p, m, n> *mat, unsigned col, unsigned from, rainbow_gf<p> s){
	rainbow_gfv<p> sv = s;
	for (unsigned i = MLPS<p,m,n>::M; i--;)
...
void rand_affine_inv(MLPS<p, n, n> *mat, MLPS<p, n, n> *inv){
	...
	rainbow_gf<p> (*mat_)[MLPS<p,n,n>::N * 8][8] = (rainbow_gf<p> (*)[MLPS<p,n,n>::N * 8][8]) mat->l;
	rainbow_gf<p> (*inv_)[MLPS<p,n,n>::N * 8][8] = (rainbow_gf<p> (*)[MLPS<p,n,n>::N * 8][8]) inv->l;
	for (unsigned j = n; j--;)
		mat_[j>>3][reorder[j]][j&7] = inv_[j>>3][reorder[j]][j&7] = (rainbow_gf<p>)1;

	for (unsigned i = n; i--;){
		rainbow_gf<p> scale = rainbow_gf<p>::rand_unit();
		mat_col_scale(mat, i, scale);
		mat_col_scale(inv, i, invert(scale));
		for (unsigned j = n; j--;){
			if (i == j)
				continue;
			scale = rainbow_gf<p>::rand();
