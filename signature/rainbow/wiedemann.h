#ifndef _RAINBOW_WIEDEMANN_H_
#define _RAINBOW_WIEDEMANN_H_
...
template <unsigned m>
void rainbow_shiftLeft( VEC<31,m> & vec )
{
...
template <unsigned m>
int rainbow_dot_without_reduce( VEC<31,m> & vec1 , VEC<31,m> & vec2 )
{
	VEC<31,m> v1 = vec1;
...
template <unsigned m>
rainbow_gf<31> rainbow_dot( const VEC<31,m> & vec1 , const VEC<31,m> & vec2 )
{
	VEC<31,m> v1 = vec1;
	...
	res = reducev( (rainbow_gfv<31>)res ).v;
	
	return ((rainbow_gfv<31>)res)[0];
}

template <unsigned n>
void rainbow_berlekamp2( VEC<31,n> & minPoly , int *deg_poly , const VEC<31,2*n> seq )
{
	...
	rainbow_gf<31> head_term,pre_head_term = 1;
	int head_deg,pre_head_deg=0;
	for( int j=0 ; j < (int)(2*n) ; j++ ){
printf("round:%d\n",j+1);
		rainbow_shiftLeft( a_i ); a_i[0] = seq[j];
printf("a_i:          [%d] " , -j ); a_i.dump( stdout );
		head_term = rainbow_dot( a_i , minPoly );
		head_deg = deg -j - 1;
...
printf("pre_min_poly: [%d] " , pre_deg ); pre_minPoly.dump( stdout );
				...
				tmp = minPoly;
				for( int i = new_deg - pre_deg ; i-- ; ) rainbow_shiftLeft( pre_minPoly );
...
printf("new minPoly:  [%d] " , deg ); minPoly.dump( stdout );
				...
				for( int i = deg - tmp_pre_deg ; i--; ) rainbow_shiftLeft( tmp );
...
template <unsigned n>
void rainbow_berlekamp_massey( VEC<31,n> & minPoly , int *deg_poly , const VEC<31,2*n> seq )
{
	VEC<31,n> a_i;	zero_vec<31,n>( &a_i );
	VEC<31,n> tmp;
	VEC<31,n> pre_minPoly;	zero_vec<31,n>( &pre_minPoly );

	zero_vec<31,n>( &minPoly ); minPoly[0] = rainbow_gf<31>(1);
...
template <unsigned n>
bool rainbow_wiedemann( VEC<31,n> & sol , const MLPS<31,n,n> &mat , const VEC<31,n> &b )
{
	VEC<31,n> bs[2*(n+1)];
	VEC<31,2*(n+1)> seq;
	VEC<31,n+1> minPoly;
	...
	for( unsigned j = 0 ; j < n ; j++ ) {
		...
		//VEC<31,n> rv;
		//rand_vec( & rv );
		//for( unsigned i = 0 ; i < 2*(n+1) ; i++ ) seq[i] = rainbow_dot( rv , bs[i] );
		//seq.dump( stdout );

		rainbow_berlekamp_massey( minPoly , & deg_poly , seq );
		//minPoly.dump( stdout );
