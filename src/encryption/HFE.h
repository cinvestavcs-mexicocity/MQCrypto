...
#include <cstdio>
#include "hfe_tower.h"
#include "hfe_groebner.h"
#include "hfe_cantor.h"
#include "hfe_bqas.h"
...
public:
	...
	static void pub_map( HFE_VEC<p,27> *y , const uint8 * pub_key , const HFE_VEC<p,27> *x );
	static size_t sizeof_pub_key() { return sizeof( HFE_MQPSv3<p,27,27> ); }

	static void gen_key( uint8 * pub_key , uint8 * pri_key );
	static void pri_pub_map( HFE_VEC<p,27> *y , const uint8 * pub_pri_key , const HFE_VEC<p,27> *x );

	static int pri_map( HFE_VEC<p,27> *x , const uint8 * pri_key , const HFE_VEC<p,27> *y );
	static size_t sizeof_pri_key() { return (sizeof( HFE_MLPS<p,27,27> ) + sizeof( HFE_MLPS<p,27,27> ) + sizeof(gf_t [30]) ); }
};

template <unsigned p>
void HFE_3_31_9<p>::pub_map( HFE_VEC<p,27> *y , const uint8 * pub_key , const HFE_VEC<p,27> *x )
{
	HFE_MQPSv3<p,27,27> poly;
	memcpy( &poly , pub_key , sizeof( poly ));
	eval( y , & poly , x );
}

template <unsigned p>
int HFE_3_31_9<p>::pri_map( HFE_VEC<p,27> *x , const uint8 * pri_key , const HFE_VEC<p,27> *y )
{
	HFE_MLPS<p,27,27> s_inv;
	HFE_MLPS<p,27,27> t_inv;
	memcpy( & s_inv , pri_key , sizeof(s_inv) );
	memcpy( & t_inv , pri_key + sizeof(s_inv) , sizeof(t_inv) );

	HFE_VEC<p,27> tmp_sec; zero_vec( & tmp_sec );
	HFE_VEC<p,27> tmp_msg; zero_vec( & tmp_msg );
...
//////////////////////////////////////////////////////////
	...
	for( int i = 0 ; i < num_z; i++ ) {
		...
		memcpy( &tmp_msg[0] , & (Y[0].v) , sizeof(hfe_gf<p>[9]));
		memcpy( &tmp_msg[9] , & (Y[1].v) , sizeof(hfe_gf<p>[9]));
		memcpy( &tmp_msg[18] , & (Y[2].v) , sizeof(hfe_gf<p>[9]));
/////////////////////////////////////////////////////////////
		eval( x , &s_inv , & tmp_msg );
		if( hfe_gf<p>(0) == (*x)[26] ) return 0;
	}
	return -1;
}

template <unsigned p>
void HFE_3_31_9<p>::pri_pub_map( HFE_VEC<p,27> *y , const uint8 * pub_pri_key , const HFE_VEC<p,27> *x )
{
	HFE_MLPS<p,27,27> s;
	HFE_MLPS<p,27,27> t;
...
///////////////////////////////////
	...
	memcpy( &tmp_sec[0] , & (Y[0].v) , sizeof(hfe_gf<p>[9]));
	memcpy( &tmp_sec[9] , & (Y[1].v) , sizeof(hfe_gf<p>[9]));
	memcpy( &tmp_sec[18] , & (Y[2].v) , sizeof(hfe_gf<p>[9]));
////////////////////////////////////
...
/////////////////////////////////////////
	...
	HFE_MQPSv3<p,l_sec,l_msg> polys;
	HFE_VEC<p,l_msg> x;
	hfe_gfv<p> _polys[HFE_VEC<p, l_sec>::M][(l_msg+1)*(l_msg+2)/2+5];
	{
		HFE_VEC<p,l_sec> y;
		zero_vec(&x);
		pri_pub_map(&y,pri_pub_key,&x);//mold->pubmap(&y, &x);
		for (unsigned k = HFE_VEC<p, l_sec>::M; k--;)
			_polys[k][0] = y.c[k];
	}

	short *_x = (short *) x.c;
	hfe_gfv<p> half = invert((hfe_gf<p>)2);
	for (unsigned a = l_msg; a--;){
		HFE_VEC<p, l_sec> yp, ym;
		_x[a] = 1;
		pri_pub_map(&yp,pri_pub_key,&x); //mold->pubmap(&yp, &x);
		_x[a] = -1;
		pri_pub_map(&ym,pri_pub_key,&x); //mold->pubmap(&ym, &x);
		_x[a] = 0;
		for (unsigned k = HFE_MQPS<p, l_sec, l_msg>::M; k--;) {
	...
	for (unsigned k = 0; k < HFE_VEC<p, l_sec>::M; k++)
		for (unsigned i = 0; i < (l_msg+1)*(l_msg+2)/2; i+=6){
			...
			if (k*2+1 != HFE_MQPSv3<p,l_sec,l_msg>::M){
...
public:
	...
	static void pub_map( HFE_VEC<p,40> *y , const uint8 * pub_key , const HFE_VEC<p,40> *x );
	static size_t sizeof_pub_key() { return sizeof( HFE_MQPSv3<p,40,40> ); }

	static void gen_key( uint8 * pub_key , uint8 * pri_key );
	static void pri_pub_map( HFE_VEC<p,40> *y , const uint8 * pub_pri_key , const HFE_VEC<p,40> *x );

	static int pri_map( HFE_VEC<p,40> *x , const uint8 * pri_key , const HFE_VEC<p,40> *y );
	static size_t sizeof_pri_key() { return (sizeof( HFE_MLPS<p,40,40> ) + sizeof( HFE_MLPS<p,40,40> ) + sizeof(gf_t [60]) ); }
};

template <unsigned p>
void HFE_4_31_10<p>::pub_map( HFE_VEC<p,40> *y , const uint8 * pub_key , const HFE_VEC<p,40> *x )
{
	HFE_MQPSv3<p,40,40> poly;
	memcpy( &poly , pub_key , sizeof( poly ));
	eval( y , & poly , x );
}

template <unsigned p>
int HFE_4_31_10<p>::pri_map( HFE_VEC<p,40> *x , const uint8 * pri_key , const HFE_VEC<p,40> *y )
{
	HFE_MLPS<p,40,40> s_inv;
	HFE_MLPS<p,40,40> t_inv;
	memcpy( & s_inv , pri_key , sizeof(s_inv) );
	memcpy( & t_inv , pri_key + sizeof(s_inv) , sizeof(t_inv) );

	HFE_VEC<p,40> tmp_sec; zero_vec( & tmp_sec );
	HFE_VEC<p,40> tmp_msg; zero_vec( & tmp_msg );
...
//////////////////////////////////////////////////////////
	...
	for( int i = 0 ; i < num_z; i++ ) {
		...
		memcpy( &tmp_msg[0] , & (Y[0].v) , sizeof( hfe_gf<p>[10]));
		memcpy( &tmp_msg[10] , & (Y[1].v) , sizeof(hfe_gf<p>[10]));
		memcpy( &tmp_msg[20] , & (Y[2].v) , sizeof(hfe_gf<p>[10]));
		memcpy( &tmp_msg[30] , & (Y[3].v) , sizeof(hfe_gf<p>[10]));
/////////////////////////////////////////////////////////////
		eval( x , &s_inv , & tmp_msg );
		if( hfe_gf<p>(0) == (*x)[39] ) return 0;
	}
	return -1;
}

template <unsigned p>
void HFE_4_31_10<p>::pri_pub_map( HFE_VEC<p,40> *y , const uint8 * pub_pri_key , const HFE_VEC<p,40> *x )
{
	HFE_MLPS<p,40,40> s;
	HFE_MLPS<p,40,40> t;
	memcpy( &s , pub_pri_key , sizeof(s));
...
///////////////////////////////////
	...
	memcpy( &tmp_sec[0] , & (Y[0].v) , sizeof(hfe_gf<p>[10]) );
	memcpy( &tmp_sec[10] , & (Y[1].v) , sizeof(hfe_gf<p>[10]) );
	memcpy( &tmp_sec[20] , & (Y[2].v) , sizeof(hfe_gf<p>[10]) );
	memcpy( &tmp_sec[30] , & (Y[3].v) , sizeof(hfe_gf<p>[10]) );
////////////////////////////////////
