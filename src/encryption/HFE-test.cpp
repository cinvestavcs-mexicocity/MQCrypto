...
)
{
	...
	*mlen = c[ENCRYPTION_BYTES-1];
	...
	uint8 b40[40];
	HFE_VEC<P,L_MSG> gf_m;
	HFE_VEC<P,L_SEC> gf_c;
	hfe_convert_to_31<12>( b40 , *((uint64*)(&c[0])) );
	hfe_convert_to_31<12>( &b40[12] , *((uint64*)(&c[8])) );
	hfe_convert_to_31<12>( &b40[24] , *((uint64*)(&c[16])) );
	hfe_convert_to_31<4>( &b40[36] , qq );
	for(int i=0;i<L_SEC;i++) gf_c[i]=b40[i];
...
//	gf_m.principle();
	...
	for( int i=0 ; i<L_MSG ; i++ ) { b40[i] = gf_m[i].v; }

	hfe_convert_from_31<13>( *((uint64*)(&m[0])) , b40 );
	hfe_convert_from_31<13>( *((uint64*)(&m[8])) , &b40[13] );
	hfe_convert_from_31<13>( *((uint64*)(&m[16])) , &b40[26] );
...
)
{
	...
	*clen = ENCRYPTION_BYTES;
	...
	hfe_convert_to_31<13>( b40 ,  *((uint64 *)(&m[0])) ); 
	hfe_convert_to_31<13>( &b40[13] ,  *((uint64 *)(&m[8])) ); 
	hfe_convert_to_31<13>( &b40[26] ,  *((uint64 *)(&m[16])) );
	b40[39] = 0;
	for(unsigned i = 0 ; i < L_MSG; i++ ) gf_m[i] = b40[i];
...
//	gf_c.principle();
	...
	for(int i=0;i<L_SEC;i++) b40[i] = gf_c[i].v;

	hfe_convert_from_31<12>( *((uint64*)(&c[0])) , b40 );
	hfe_convert_from_31<12>( *((uint64*)(&c[8])) , &b40[12] );
	hfe_convert_from_31<12>( *((uint64*)(&c[16])) , &b40[24] );
	union {
	uint64 qq;
	uint8 bb[8];
	};
	qq = 0;
	hfe_convert_from_31<4>( qq , &b40[36] );
	c[24] = bb[0];
	c[25] = bb[1];
	c[26] = bb[2];
...
int main()
{
	...
	hfe_encrypt( c , & lc , m ,  0 , pk , lpk );
	for( int i=0;i<ENCRYPTION_BYTES;i++) printf("%x " , (unsigned) c[i]); printf("\n");
	int q = hfe_decrypt( cm , & lcm , c , lc , sk , lsk );
	...
	hfe_encrypt( c , & lc , m ,  lm , pk , lpk );
	for( int i=0;i<ENCRYPTION_BYTES;i++) printf("%x " , (unsigned) c[i]); printf("\n");
	q = hfe_decrypt( cm , & lcm , c , lc , sk , lsk );
	for( int i=0;i<SHORTPLAINTEXT_BYTES;i++) 
		printf("%x " , (unsigned) cm[i]); printf(": %d\n", q );
...
int main(){
	...
	HFE_VEC<P, L_MSG> vec0, vec2;//, vec4;
	HFE_VEC<P, L_SEC> vec1;//, vec3;//, vec5;
