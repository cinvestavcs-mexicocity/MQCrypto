#ifndef _TTS6440_WIEDEMANN_H_
#define _TTS6440_WIEDEMANN_H_
...
template <unsigned m>
void shiftLeft( tts6440_VEC<31,m> & vec )
{
	for(unsigned i=tts6440_VEC<31,m>::M-1;i>0;i--)
		vec.c[i].v = _mm_or_si128(_mm_srli_si128( vec.c[i-1].v,14),_mm_slli_si128(vec.c[i].v,2));
	vec.c[0].v = _mm_slli_si128(vec.c[0].v,2);
}

template <unsigned m>
int dot_without_reduce( tts6440_VEC<31,m> & vec1 , tts6440_VEC<31,m> & vec2 )
{
	tts6440_VEC<31,m> v1 = vec1;

	if( 0 != (m%8))
		v1.c[tts6440_VEC<31,m>::M-1].v = _mm_srli_si128( _mm_slli_si128( (v1.c[tts6440_VEC<31,m>::M-1].v),(2*(8-(m%8))) ), (2*(8-(m%8))) );
	__m128i res = _mm_setzero_si128();
	for(unsigned i=tts6440_VEC<31,m>::M;i--;) res = _mm_add_epi32(res, _mm_madd_epi16(v1.c[i].v,vec2.c[i].v));
...
template <unsigned n>
void berlekamp2( tts6440_VEC<31,n> & minPoly , int *deg_poly , const tts6440_VEC<31,2*n> seq )
{
	tts6440_VEC<31,n> a_i; zero_vec<31,n>( &a_i );
	tts6440_VEC<31,n> tmp;
	tts6440_VEC<31,n> pre_minPoly; zero_vec<31,n>( & pre_minPoly );
	zero_vec<31,n>( &minPoly ); ((short*)( &minPoly))[0] = 1;
	int pre_deg=0 , deg=0;
	
	tts6440_gf<31> head_term,pre_head_term = 1;
...
template <unsigned n>
void berlekamp_massey( tts6440_VEC<31,n> & minPoly , int *deg_poly , const tts6440_VEC<31,2*n> seq )
{
	tts6440_VEC<31,n> a_i;	zero_vec<31,n>( &a_i );
	tts6440_VEC<31,n> tmp;
	tts6440_VEC<31,n> pre_minPoly;	zero_vec<31,n>( &pre_minPoly );

	zero_vec<31,n>( &minPoly ); minPoly[0] = tts6440_gf<31>(1);
...
template <unsigned n>
bool wiedemann( tts6440_VEC<31,n> & sol , const MLPS<31,n,n> &mat , const tts6440_VEC<31,n> &b )
{
	tts6440_VEC<31,n> bs[2*(n+1)];
	tts6440_VEC<31,2*(n+1)> seq;
	tts6440_VEC<31,n+1> minPoly;
	tts6440_VEC<31,n+1> backMinPoly;
	int deg_back = 0;
	int deg_poly = 0;
